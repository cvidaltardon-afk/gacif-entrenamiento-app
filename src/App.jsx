        ...
        <div className={`mb-3 rounded-xl border p-3 flex items-center justify-between gap-2 ${
          ...
          {canEdit ? (
            <>
              ...
              <button onClick={() => setEditWeek(null)} className="shrink-0 px-3 py-2 rounded-lg bg-gacif-800 border border-gacif-border text-xs font-bold text-gacif-light">
                Bloquear
              </button>
            </>
          ) : (
            <>
              <div>
                ...
                <p className="text-[11px] text-gacif-gray-dark mt-0.5">Desbloquea para editar.</p>
              </div>
              <ConfirmButton
                label="✎ Editar"
...
function Btn({ children, onClick, disabled }) {
  return (
    <button onClick={onClick} disabled={disabled} className={`w-8 h-8 rounded-lg bg-gacif-800 text-gacif-light font-black text-base active:bg-gacif-900 ${disabled ? "opacity-60 cursor-default" : ""}`}>
      {children}
    </button>
  );
}
    ...
    <section className={`mb-3 rounded-2xl border p-3 ${c.done ? "border-emerald-700 bg-emerald-950" : "border-gacif-border bg-gacif-900"}`}>
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        <button
          ...
          disabled={!isCurrentWeek}
          className={`px-4 h-10 rounded-lg font-black text-sm ${c.done ? "bg-emerald-500 text-gacif-bg" : "bg-gacif-800 text-gacif-gray border border-gacif-border"}`}
        >
          {c.done ? "✓ Hecho" : "Marcar"}
        </button>
      </div>
              ...
              <div style={{ width: "100%", height: 200 }}>
                <ResponsiveContainer>
                  <LineChart data={series}>
                    ...
                    <Line type="monotone" dataKey="maxKg" stroke="#C0C0C0" strokeWidth={2.5} dot={{ r: 4, fill: "#C0C0C0" }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
      ...
      {weeks.length === 0 ? (
        ...
        <>
          <div className="flex gap-2 mb-3">
            ...
            <button onClick={copy} className={`px-4 rounded-xl font-black text-sm ${copied ? "bg-emerald-500 text-gacif-bg" : "bg-gacif-plata text-gacif-bg"}`}>
              {copied ? "✓" : "Copy"}
            </button>
          </div>
          <textarea
